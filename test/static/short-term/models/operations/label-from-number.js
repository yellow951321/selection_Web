...
import {User, } from 'short-term/models/association.js'

import {map, shortTermFromNumber, } from 'projectRoot/lib/static/javascripts/mapping/label.js'

const expect = chai.expect

describe('test short-term/models/operations/label-from-number.js', ()=>{
      ...
      for(let aspectIndex of [0, 1, 2, 3, 4, ]){
        for(let keypointIndex in map[aspectIndex].keypoint){
          for(let methodIndex in map[aspectIndex].keypoint[keypointIndex].method){
            ...
            expected.method = shortTermFromNumber({aspect: aspectIndex, keypoint: keypointIndex, method: methodIndex, }).method
            expected.keypoint = shortTermFromNumber({aspect: aspectIndex, keypoint: keypointIndex, }).keypoint
            expected.aspect = shortTermFromNumber({aspect: aspectIndex, }).aspect
